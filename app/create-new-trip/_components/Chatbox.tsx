...
const Chatbox = () => {
    ...
    const onSend = async() => {
        ...
        const result  = await axios.post('/api/aimodel', { messages : [...messages, newMsg] , isFinal: isFinal});

        // console.log("TRIP PLAN FINAL:", result?.data);

        if(result?.data?.reason?.type == "RATE_LIMIT"){
            setLoading(false);
            return setMessages((prev) => [...prev, { role: 'assistant', content: `You have reached the limit for today. Please try again tomorrow.` }]);
        }

        !isFinal && setMessages((prev) => [...prev, { role: 'assistant', content: result.data.resp , ui: result?.data?.ui}]);
    ...
    const RenderGenerativeUi = (ui:string) => {
        ...
        if(ui === 'budget') {
            return <div><BudgetUI onSelectedOption={(v:string)=> {setUserInput(v.trim()); onSend()}}/></div>
        }
        else if(ui === 'groupSize') {
            return <GroupSizeUi onSelectedOption={(v:string)=> {setUserInput(v.trim()); onSend()}}/>
        }
        else if(ui === 'tripDuration') {
            return <div><DurationUI
            initialDays={4}
            onDone={(d) => setUserInput(`${d} Days`)} // fired when Done clicked
            /></div>
        }
        else if(ui === 'travelInterests'){
            return <div><TravelInterestUI onSelectedOption={(s) => setUserInput("My interest(s): "+ s)}/></div>
        }
        else if(ui === 'final') {
            return <div><FinalUI viewTrip={tripDetails} disable={!tripDetails}/></div>
        }
        return null;
    }
    ...
    <div className='h-[87vh] flex flex-col border border-gray-300 rounded-lg'>
            ...
            {messages.map((msg: Message, index) => (
                        msg.role == 'user' ? (
                            <div key={index} className='flex justify-end my-4'>
                                <div className='max-w-lg bg-sky-600 text-white px-4 py-2 rounded-lg shadow'>{msg.content}</div>
                            </div>
                        ) : (
        ...
        <section>
            <div className='flex relative'>
                <Textarea disabled={loading} placeholder='Start typing here...' className='w-full h-28 bg-transparent border border-gray-200 m-4' onChange={(e)=> setUserInput(e.target.value)} value={userInput}/> 
                <Button disabled={loading} className='absolute right-8 bottom-8' size={'icon'} onClick={() =>onSend()}>
                    <Send />
                </Button>
            </div>
        </section>
