...
const dynamic = "force-dynamic"; // avoid caching when testing

type Body = { placeName?: string; placeId?: string };

export async function POST(req: Request) {
  try {
    const { placeName, placeId }: Body = await req
      .json()
      .catch(() => ({}) as Body);
    ...
    const detailsResp = await fetch(
      `https://places.googleapis.com/v1/places/${id}`,
      {
        ...
        headers: {
          "X-Goog-Api-Key": apiKey,
          "X-Goog-FieldMask": ["id", "displayName", "location", "photos"].join(
            ","
          ),
        },
        cache: "no-store",
    ...
    const firstPhoto =
      detailsJson?.photos && detailsJson.photos.length > 0
        ? detailsJson.photos[0]
        : null;
    if (firstPhoto) {
      ...
      photoUrl =
        ...
        firstPhoto?.authorAttributions?.[0]?.photoUri ??
        null;
    }
